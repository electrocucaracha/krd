...
function _run_assertions {
    ...
    if [[ ${HOST_INSTALLER:-false} == "true" ]]; then
        ...
        assert_are_equal "${KRD_KUBESPRAY_VERSION:-v2.22.1}" "$(git describe --abbrev=0 --tags)"
        popd >/dev/null
    else
        ...
        assert_contains "$($VAGRANT_CMD_SSH_INSTALLER "cd /opt/kubespray; git describe --abbrev=0 --tags")" "${KRD_KUBESPRAY_VERSION:-v2.22.1}"
    fi
}
