...
function _run_assertions {
    ...
    if [[ "${HOST_INSTALLER:-false}" == "true" ]]; then
        ...
        assert_are_equal "${KRD_KUBE_VERSION:-v1.23.1}" "$(kubectl version --short | awk 'FNR==2{print $3}')"
        pushd /opt/kubespray > /dev/null
        assert_are_equal "${KRD_KUBESPRAY_VERSION:-v2.18.0}" "$(git describe --abbrev=0 --tags)"
        popd > /dev/null
    else
        assert_contains "$($VAGRANT_CMD_SSH_INSTALLER "command -v kubectl")" "kubectl"
        assert_contains "$($VAGRANT_CMD_SSH_INSTALLER "kubectl version --short | awk 'FNR==2{print \$3}'")" "${KRD_KUBE_VERSION:-v1.23.1}"
        assert_contains "$($VAGRANT_CMD_SSH_INSTALLER "cd /opt/kubespray; git describe --abbrev=0 --tags")" "${KRD_KUBESPRAY_VERSION:-v2.18.0}"
    fi
}
