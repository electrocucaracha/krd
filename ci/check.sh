...
function _run_assertions {
    ...
    if [[ ${HOST_INSTALLER:-false} == "true" ]]; then
        ...
        assert_are_equal "${KRD_KUBE_VERSION:-v1.31.4}" "$(kubectl version -o yaml | grep gitVersion | awk 'FNR==2{ print $2}')"
        ...
        assert_contains "$($VAGRANT_CMD_SSH_INSTALLER "kubectl version -o yaml | grep gitVersion | awk 'FNR==2{ print \$2}'")" "${KRD_KUBE_VERSION:-v1.31.4}"
        assert_contains "$($VAGRANT_CMD_SSH_INSTALLER "cd /opt/kubespray; git describe --abbrev=0 --tags")" "${KRD_KUBESPRAY_VERSION:-v2.28.0}"
    fi
}
