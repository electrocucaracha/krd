...
assert_contains "$(kubectl logs -n istio-system "$(kubectl get pods -n istio-system -l app=istiod -o jsonpath='{.items[*].metadata.name}')")" "Process sidecar injection request" "The Client's sidecar injection request wasn't received"
